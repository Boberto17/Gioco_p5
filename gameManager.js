...
function createMap() {
    // Rimuovi il fill grigio precedente
    noStroke();
    for (let i = 0; i < width; i += tileSize) {
        for (let j = 0; j < height; j += tileSize) {
            if (isBuildable(i, j)) {
                // Evidenzia zone costruibili con un colore più chiaro e trasparente
                fill(100, 200, 100, 100); // Verde chiaro semi-trasparente
                rect(i, j, tileSize, tileSize);
                
                // Aggiungi un bordo sottile per delimitare meglio l'area
                stroke(50, 150, 50, 200);
                strokeWeight(2);
                noFill();
                rect(i, j, tileSize, tileSize);
            }
...
function handleTowerPlacement() {
    // Trova il centro del tile più vicino
    let tileX = Math.floor(mouseX / tileSize) * tileSize + tileSize / 2;
    let tileY = Math.floor(mouseY / tileSize) * tileSize + tileSize / 2;
    ...
    if (selectedTowerType.current && gold >= selectedTowerType.current.prototype.cost) {
        if (isBuildable(tileX, tileY)) {
            ...
            switch(selectedTowerType.current) {
                case FireTower:
                    newTower = new FireTower(tileX, tileY);
                    break;
                case IceTower:
                    newTower = new IceTower(tileX, tileY);
                    break;
                case SniperTower:
